...
export const exportToCSV = async (data: any[], baseFilename: string) => {
  try {
    ...
    if (isCapacitor()) {
      await Filesystem.writeFile({ path: fileName, data: csv, directory: Directory.Cache });
      const uri = await Filesystem.getUri({ path: fileName, directory: Directory.Cache });
      await Share.share({ title: 'Export CSV', text: `Fichier ${baseFilename}.csv`, url: uri.uri });
    } else {
